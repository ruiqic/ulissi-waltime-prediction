...
def readFile(filename):
    ...
    Returns empty string when UnicodeDecodeError is encountered.
...
def get_steptimes(launch_dir):
    """
    Obtain the time per ionic step for each step of the given calculation.
    ...
    Returns:
        steptimes (list): contains the the time for each step in seconds.
    """
    try:
        for file in os.listdir(launch_dir):
            if fnmatch.fnmatch(file, 'OUTCAR*'):
                ...
                if m:
                    steptimes = list(map(lambda x: float(x), m))
                    return steptimes
                else:
                    raise ParsefileFail  
    except FileNotFoundError:
...
def get_avg_steptime(launch_dir):
    ...
    """
    steptimes = get_steptimes(launch_dir)
